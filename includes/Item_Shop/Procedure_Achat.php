...
function Verification_Place_Inventaire_IS($Verification_Place_Account_Id, $Connexion, $Procedure_Achat_Item_Nombre = 1) {
...
$Verification_Item_ID = "SELECT id FROM site.itemshop 
                                   ...
                                   AND actif = ?
                                   LIMIT 1";
$Parametres_Verification_Item_ID = $Connexion->prepare($Verification_Item_ID);
...
if ($Nombre_De_Resultat_Verification_Item_ID == 1) {
    ...
    $Recuperation_Information = "SELECT * FROM site.itemshop 
                                   ...
                                   AND actif = ?
                                   LIMIT 1";
    $Parametres_Recuperation_Information = $Connexion->prepare($Recuperation_Information);
    ...
    if ($Donnees_Recuperation_Information->type == 1) {
        ...
        if ($_SESSION['VamoNaies'] >= ($Procedure_Achat_Item_Prix * $Procedure_Achat_Parametres_Item_Nombre)) {
            ...
            $Recuperation_Flag = "SELECT flag FROM player.item_proto 
                                              WHERE vnum = ?
                                              LIMIT 1";
            $Parametres_Recuperation_Flag = $Connexion->prepare($Recuperation_Flag);
            ...
            if ($Procedure_Achat_Item_Flac == 4 ||
                ...
                if (is_numeric(Verification_Place_Inventaire_IS($Procedure_Achat_User_ID, $Connexion))) {

                    $Index_Position = Verification_Place_Inventaire_IS($Procedure_Achat_User_ID, $Connexion);
        ...
        if ($_SESSION['VamoNaies'] >= $Procedure_Achat_Item_Prix) {
            ...
            if ($Procedure_Achat_Item_Nombre == 9999) {
                ...
                $Recuperation_Bonus = "SELECT $bonus 
                                                ...
                                                AND $bonus > NOW()
                                                LIMIT 1";
                $Parametres_Recuperation_Bonus = $Connexion->prepare($Recuperation_Bonus);
