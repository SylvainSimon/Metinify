...
if (!empty($_SESSION["ID"])) {
    ...
    $Recuperation_Marche_Personnage = "SELECT *
                                       ...
                                       WHERE id = ?
                                       LIMIT 1";
    $Parametres_Recuperation_Marche_Personnage = $Connexion->prepare($Recuperation_Marche_Personnage);
    ...
    <?php if ($Nombre_De_Resultat_Recuperation_Marche_Personnage != 0) { ?>
        ...
        $Recuperation_Monnaies = "SELECT *
                                  ...
                                  WHERE id = ?
                                  LIMIT 1";
        $Parametres_Recuperation_Monnaies = $Connexion->prepare($Recuperation_Monnaies);
        ...
        $Recuperation_Article = "SELECT *
                                  ...
                                  LIMIT 1";
        $Parametres_Recuperation_Article = $Connexion->prepare($Recuperation_Article);
                    ...
                    <?php if ($PID_Emplacement_Disponible != "") { ?>
                        ...
                        <?php if ($Nombre_De_Ligne_Supprimes != 0) { ?>
                            ...
                            <?php if ($Devise_Prix == 1) { ?>
                                ...
                                $Update_Monnaies_Vendeur = "UPDATE $BDD_Account.account
                                          SET cash = (cash + $Prix_Article)
                                          WHERE id = ?
                                          LIMIT 1";

                                $Parametres_Update_Monnaies_Vendeur = $Connexion->prepare($Update_Monnaies_Vendeur);
                                ...
                                $Update_Monnaies_Acheteur = "UPDATE $BDD_Account.account
                                          SET cash = (cash - $Prix_Article)
                                          WHERE id = ?
                                          LIMIT 1";

                                $Parametres_Update_Monnaies_Acheteur = $Connexion->prepare($Update_Monnaies_Acheteur);
                            ...
                            /* --------------------------- Insertion de l'item ---------------------------- */
                            $Insertion_Logs = "INSERT INTO $BDD_Site.log_achats (id_compte, compte, vnum_item, item, quantite, prix, monnaie, date, ip, resultat) 
                            ...
                            /* --------------------------- Insertion de l'item ---------------------------- */
                            $Insertion_Logs_Marche = "INSERT INTO $BDD_Site.logs_marche_achats (id_vendeur, id_acheteur, id_personnage, prix, devise, date, ip) 
                            ...
                            $Update_Player_Index_Acheteur = "UPDATE $BDD_Player.player_index 
                                                             SET $PID_Emplacement_Disponible = ?
                                                             WHERE id = ?
                                                             LIMIT 1";

                            $Parametres_Update_Player_Index_Acheteur = $Connexion->prepare($Update_Player_Index_Acheteur);
                            ...
                            $Update_Player_Index_Vendeur = "UPDATE $BDD_Player.player_index 
                                                            SET $PID_Emplacement_Vendeur = '0'
                                                            WHERE id = ?
                                                            LIMIT 1";

                            $Parametres_Update_Player_Index_Vendeur = $Connexion->prepare($Update_Player_Index_Vendeur);
