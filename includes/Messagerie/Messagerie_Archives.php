...
$Boite_Reception = "SELECT DISTINCT(numero_discussion) FROM $BDD_Site.support_ticket_archives
                             WHERE id_recepteur = ?
                             OR id_emmeteur = ?";
...
/* ------------------------------------- Recuperation discussions  ------------------------------------ */
$Boite_Reception_Message_Total = "SELECT COUNT(id) AS nombre FROM $BDD_Site.support_ticket_archives
                                  WHERE id_recepteur = ?
                                  OR id_emmeteur = ?";
...
<div class="Zone_Archives">
    <table class="Tableau_Archives">
        ...
        $Listage_Fils = "SELECT numero_discussion 
                         ...
                         ORDER by date DESC";
        $Parametres_Listage_Fils = $Connexion->prepare($Listage_Fils);
        ...
        $Pseudo_Messagerie_Emmeteur = "SELECT pseudo_messagerie 
                                       ...
                                       WHERE id = ?
                                       LIMIT 1";
        $Parametres_Pseudo_Messagerie_Emmeteur = $Connexion->prepare($Pseudo_Messagerie_Emmeteur);
        /* -------------------------------------------------------------------------- */
        ...
        $Pseudo_Messagerie_Recepteur = "SELECT pseudo_messagerie 
                                        ...
                                        WHERE id = ?
                                        LIMIT 1";
        $Parametres_Pseudo_Messagerie_Recepteur = $Connexion->prepare($Pseudo_Messagerie_Recepteur);
        /* -------------------------------------------------------------------------- */
        ?>
        ...
        <tbody>
            ...
            <?php if ($Nombre_De_Resultat_Listage_Fils != 0) { ?>
                ...
                <?php while ($Donnees_Listage_Fils = $Parametres_Listage_Fils->fetch()) { ?>
                    ...
                    $Recuperation_Premier_Message = "SELECT * FROM $BDD_Site.support_ticket_archives
                                                     WHERE numero_discussion = ?
                                                     ORDER by date ASC";
                    $Parametres_Recuperation_Premier_Message = $Connexion->prepare($Recuperation_Premier_Message);
