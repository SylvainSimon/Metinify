...
$Boite_Reception = "SELECT DISTINCT(numero_discussion) FROM $BDD_Site.support_ticket_traitement
                             WHERE id_recepteur = ?
                             ORDER by date DESC";
$Parametres_Boite_Reception = $Connexion->prepare($Boite_Reception);
...
/* ------------------------------------- Recuperation discussions  ------------------------------------ */
$Boite_Reception_Message_Total = "SELECT COUNT(id) AS nombre FROM $BDD_Site.support_ticket_traitement
                                  WHERE id_recepteur = ?
                                  OR id_emmeteur = ?";
$Parametres_Boite_Reception_Message_Total = $Connexion->prepare($Boite_Reception_Message_Total);
...
/* ------------------------------------- Recuperation discussions  ------------------------------------ */
$Boite_Reception_Message_Non_Lu = "SELECT COUNT(id) AS nombre FROM $BDD_Site.support_ticket_traitement
                                  WHERE id_recepteur = ?
                                  AND etat= 'Non-Lu'";
$Parametres_Boite_Reception_Message_Non_Lu = $Connexion->prepare($Boite_Reception_Message_Non_Lu);
...
$Boite_Reception_Dernier_Message_NonLu = "SELECT * FROM $BDD_Site.support_ticket_traitement
                                          WHERE numero_discussion = ?
                                          AND etat = 'Non-Lu'
                                          AND id_emmeteur != ?
                                          ORDER by date ASC
                                          LIMIT 1";
$Parametres_Reception_Dernier_Message_NonLu = $Connexion->prepare($Boite_Reception_Dernier_Message_NonLu);
/* -------------------------------------------------------------------------- */

/* ------------------------ Recuperation dernier message  ---------------------------- */
$Boite_Reception_Dernier_Message = "SELECT * FROM $BDD_Site.support_ticket_traitement
                             WHERE numero_discussion = ?
                             AND id_emmeteur != ?
                             ORDER by date DESC
                             LIMIT 1";
$Parametres_Reception_Dernier_Message = $Connexion->prepare($Boite_Reception_Dernier_Message);
/* -------------------------------------------------------------------------- */
...
<table id="Tableau_Recapitulatif_Boite_Messagerie">
    <tr>
    ...
    <tr>
        <td class="Colonne_Gauche" title="Nombre de message non-lu dans la boite de réception">Non-lu :</td>
